---
name: westockdata
description: WeStock Data 数据源：腾讯自选股行情数据接口 | 支持市场：A股（沪深/科创/北交所）、港股、美股
...
- **包来源**：westock-data-clawhub@1.0.4 发布于npm官方镜像（registry.npmjs.org），maintainer 为 腾讯自选股团队成员。
- **完整性校验**：包的 sha512 integrity 为 `sha512-Cr4IS69wJ6aFdaDv7Sh/Zwf1FEj+8BHxegIltjWg4bswjV2SfbG9VmM0YN4SwfaLJlP1INzM0Ed3LXP+3WpjSA==`，shasum 为 `b434e6ca4b434455201f1d8af56da435f518b678`。
- **执行方式**：`npx -y` 会自动下载并执行该包，属于中等供应链风险。在敏感/生产环境使用前，建议向 IT 安全团队确认。
...
详细返回格式见 `references/ai_usage_guide.md`
...
完整分析场景参见 `references/scenarios-guide.md`

详细返回格式、字段说明、分析模板参见 `references/ai_usage_guide.md`

## 重要声明

⚠️ **重要声明**：

- 本技能仅提供客观市场数据的查询与展示服务，所有返回数据均来源于公开市场信息，不含任何主观分析、投资评级或交易建议。
- 本技能不构成证券投资咨询服务，使用本技能获取的数据不应作为投资决策的唯一依据。
