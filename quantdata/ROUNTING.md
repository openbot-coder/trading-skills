...
| **加密货币** | XXX-YYY | BTC-USD | ccxtdata ⭐ | coingeckodata / yfinancedata / okxdata / binancedata | 主流加密货币，100+交易所 |
...
| **宏观数据(中债)** | bond / curve | macro_aux.py bond | macrodata ⭐ | --- | 中美国债收益率，akshare 直连可用 |
| **宏观数据(外汇)** | fx | macro_aux.py fx | macrodata ⭐ | --- | 外汇汇率，akshare 直连可用 |
| **宏观数据(VIX)** | vix | macro_aux.py vix | macrodata | yfinancedata | VIX，海外网络可用 |
| **宏观数据(DXY)** | dxy | macro_aux.py dxy | macrodata | yfinancedata | 美元指数，海外网络可用 |
| **北向资金** | flow | northbound.py | capitalflowdata ⭐ | westockdata | 沪深港通资金流向 |
| **申万行业** | sector / pe | sector.py | sectordata ⭐ | --- | 申万行业行情/估值 |
...
#### 2.6. 加密货币衍生品 (Crypto Derivatives)

| 市场 | 数据类型 | ccxtdata | okxdata | binancedata | coingeckodata | yfinancedata | 说明 |
|-----|---------|----------|---------|-------------|--------------|----------|------|
| **加密货币** | **实时价格** | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ✅ 支持 | ✅ 支持 | ccxtdata优先，100+交易所统一 |
| **加密货币** | K线数据 | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ✅ 支持 | ✅ 支持 | ccxtdata统一接口 |
| **加密货币** | 深度数据 | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ❌ 不支持 | ❌ 不支持 | ccxtdata统一深度 |
| **加密货币** | 资金费率 | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ❌ 不支持 | ❌ 不支持 | 永续合约资金费率 |
| **加密货币** | 历史成交 | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ❌ 不支持 | ❌ 不支持 | 最新成交记录 |
| **加密货币** | 永续合约 | ✅ **支持 ⭐** | ✅ 支持 | ✅ 支持 | ❌ 不支持 | ❌ 不支持 | ccxtdata/USDT:USDT格式 |
| **加密货币** | 期权 | ✅ 支持 | ✅ 支持 | ✅ 支持 | ❌ 不支持 | ❌ 不支持 | Deribit等期权交易所 |
...
| **中美国债收益率** | 中国/美国 2Y/5Y/10Y/30Y | `macrodata ⭐` | akshare bond_zh_us_rate |
| **中国国债收益率曲线** | 全期限中国国债 | `macrodata ⭐` | akshare bond_china_yield |
| **外汇实时汇率** | 25个货币对 (USD/CNY等) | `macrodata ⭐` | akshare fx_spot_quote |
| **VIX 恐慌指数** | 美股波动率指数 | `macrodata` | yfinance ^VIX（海外） |
| **美元指数 DXY** | 美元贸易加权指数 | `macrodata` | yfinance DX-Y.NYB（海外） |
| **美国国债收益率** | FRED 10Y/2Y/30Y | `macrodata` | FRED（海外不稳定） |

#### I. 北向资金数据 (Capital Flow)

| 数据类型 | 详细描述 | 路由目标 | 说明 |
|---------|---------|---------|------|
| **北向资金流向** | 沪深港通北向资金净流入 | `capitalflowdata ⭐` | akshare 东方财富 |
| **北向历史数据** | 北向资金历史日线 | `capitalflowdata ⭐` | akshare 东方财富 |
| **行业资金排名** | 北向资金行业分布 | `capitalflowdata ⭐` | akshare 东方财富 |
| **南向资金流向** | 港股通南向资金净流入 | `capitalflowdata ⭐` | akshare 东方财富 |

#### J. 行业板块数据 (Sector)

| 数据类型 | 详细描述 | 路由目标 | 说明 |
|---------|---------|---------|------|
| **申万行业实时** | 124+ 二级行业实时行情 | `sectordata ⭐` | akshare index_realtime_sw |
| **申万行业历史** | 申万行业历史日线 | `sectordata ⭐` | akshare index_hist_sw |
| **申万行业估值** | PE/PB 市盈率市净率 | `sectordata ⭐` | akshare stock_sector_spot |
| **板块行情** | 49 个板块涨跌排行 | `sectordata ⭐` | akshare stock_sector_spot |
...
skills:
  ...
  macrodata:
    ...
    data_types: [macro, economic_calendar, policy, bond_yield, fx_rate, vix, dollar_index]
    retry:
      max_attempts: 3
      backoff: exponential
      initial_delay: 1s
  ...
  capitalflowdata:
    path: capitalflowdata
    enabled: true
    priority: 7
    markets: [china, hk]
    data_types: [northbound_flow, southbound_flow, sector_flow]
    retry:
      max_attempts: 3
      backoff: exponential
      initial_delay: 1s
  ...
  sectordata:
    ...
    priority: 7
    markets: [china]
    data_types: [sector_realtime, sector_hist, sector_valuation]
    retry:
      max_attempts: 3
      backoff: exponential
      initial_delay: 1s
  ...
  ccxtdata:
    ...
    enabled: true
    priority: 9
    markets: [crypto, crypto_futures, crypto_options]
    data_types: [quote, kline, depth, funding_rate, trades]
    retry:
      max_attempts: 3
      backoff: exponential
      initial_delay: 1s
...
```
1. 加密货币全类型 → ccxtdata（100+交易所统一接口）
2. 实时数据 → westockdata（A股）
3. 国际期货 → crawldata
4. 技术指标 → westockdata
5. 资金流向 → westockdata
6. 港股/美股 → yfinancedata
7. 宏观经济 → baostockdata
8. 杜邦分析 → baostockdata
9. 长周期K线 → baostockdata
10. A股K线 → westockdata（优先）
11. 中美国债收益率 / 外汇汇率 → macrodata（akshare，国内正常）
12. VIX / 美元指数 → macrodata（yfinance，海外部署后生效）
13. 北向资金流向 → capitalflowdata（akshare 东方财富）
14. 申万行业行情 / 估值 → sectordata（akshare）
```
...
| 数据 | 模块 | 脚本 | 数据源 | 可用性 | 示例 |
|------|------|------|--------|--------|------|
| 中美国债收益率 | macrodata | `macro_aux.py bond` | akshare | ✅ 国内直接访问 | `python macro_aux.py bond` |
| 中国国债收益率曲线 | macrodata | `macro_aux.py curve` | akshare | ✅ 国内直接访问 | `python macro_aux.py curve` |
| 外汇汇率 (对CNY) | macrodata | `macro_aux.py fx` | akshare | ✅ 国内直接访问 | `python macro_aux.py fx` |
| VIX 恐慌指数 | macrodata | `macro_aux.py vix` | yfinance | ⚠️ 超时容错(默认10s) | `python macro_aux.py vix --timeout 10` |
| 美元指数 DXY | macrodata | `macro_aux.py dxy` | yfinance | ⚠️ 超时容错(默认10s) | `python macro_aux.py dxy --timeout 10` |
| FRED 系列(VIXCLS等) | macrodata | `macro_aux.py fred` | FRED CSV | ⚠️ 超时容错(默认45s) | `python macro_aux.py fred --series VIXCLS` |
| 融资融券余额 | macrodata | `macro_aux.py margin` | akshare | ✅ 国内直接访问 | `python macro_aux.py margin -n 5` |
| 恐贪指数 | macrodata | `macro_aux.py fear-greed` | alternative.me | ✅ 免费无 key | `python macro_aux.py fear-greed -n 5` |

### A股分析师一致预期

| 数据 | 模块 | 脚本 | 数据源 | 可用性 | 示例 |
|------|------|------|--------|--------|------|
| 分析师一致预期 | analystdata | `analyst.py forecast` | akshare | ✅ 国内直接访问 | `python analyst.py forecast` |
| 个股研报明细 | analystdata | `analyst.py detail <symbol>` | akshare | ✅ 国内直接访问 | `python analyst.py detail 600519` |

### 公司事件数据

| 数据 | 模块 | 脚本 | 数据源 | 可用性 | 示例 |
|------|------|------|--------|--------|------|
| 上交所增减持 | corpdata | `corp.py share_change --market sse` | akshare | ✅ 国内直接访问 | `python corp.py share_change --market sse` |
| 深交所增减持 | corpdata | `corp.py share_change --market szse` | akshare | ✅ 国内直接访问 | `python corp.py share_change --market szse` |
| 定向增发(定增) | corpdata | `corp.py placement <symbol>` | akshare | ✅ 国内直接访问 | `python corp.py placement 600519` |

### 北向资金

| 数据 | 模块 | 脚本 | 数据源 | 可用性 | 示例 |
|------|------|------|--------|--------|------|
| 沪深港股通资金 | capitalflowdata | `northbound.py` | akshare | ✅ 国内直接访问 | `python northbound.py` |
| 北向资金明细 | capitalflowdata | `northbound.py --detail` | akshare | ✅ 国内直接访问 | `python northbound.py --detail` |

### 申万行业指数

| 数据 | 模块 | 脚本 | 数据源 | 可用性 | 示例 |
|------|------|------|--------|--------|------|
| 申万行业估值 | sectordata | `sector.py` | akshare | ✅ 国内直接访问 | `python sector.py` |
| 行业热度分析 | sectordata | `sector.py --heat` | akshare | ✅ 国内直接访问 | `python sector.py --heat` |

### Vibe-Trading Skill 路由对应

| Vibe-Trading Skill | 需要的数据 | 路由到 | 命令 |
|-------------------|-----------|--------|------|
| sector-rotation | 行业指数估值、热度 | sectordata sector | `sector.py` |
| macro-analysis | 国债收益率 | macrodata macro_aux | `macro_aux.py bond` |
| global-macro | VIX、DXY | macrodata macro_aux | `macro_aux.py vix/dxy` |
| commodity-analysis | 外汇汇率 | macrodata macro_aux | `macro_aux.py fx` |
| sentiment-analysis | 北向资金流向 | capitalflowdata northbound | `northbound.py` |
| sentiment-analysis | VIX 恐慌指数 | macrodata macro_aux | `macro_aux.py vix` |
| earnings-forecast | 分析师一致预期 | analystdata | `analyst.py forecast` |
| earnings-revision | 分析师评级变动 | analystdata | `analyst.py forecast` |
| credit-analysis | 国债收益率 | macrodata macro_aux | `macro_aux.py bond` |
| corporate-events | 增减持/定增 | corpdata | `corp.py share_change / placement` |
| fundamental-filter | 盈利预测 EPS | analystdata | `analyst.py forecast` |
| valuation-model | 行业估值 PE/PB | sectordata | `sector.py` |
| financial-statement | 财务报表 | baostockdata | baostock A股 |
| event-driven | 股东增减持 | corpdata | `corp.py share_change` |
| sentiment-analysis | 融资融券余额 | macrodata macro_aux | `macro_aux.py margin` |
| sentiment-analysis | 恐贪指数 | macrodata macro_aux | `macro_aux.py fear-greed` |
